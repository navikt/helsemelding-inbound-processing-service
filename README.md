...
- Publishing to the output topic (`helsemelding.dialog.in`) can fail (e.g. broker/network issues) which 
results in a `StreamsException` from Kafka Streams.

**Intended behavior**
- When conversion fails the topology throws an exception and the problematic record is not forwarded to the output topic. 
The underlying issue must be fixed and the `helsemelding-inbound-processing-service` redeployed before previously failed records can be processed.
